...
namespace DigitalsiteSaaS\Comunidad\Http;
use App\Http\Controllers\Controller;
use DigitalsiteSaaS\Comunidad\Nota;
use DigitalsiteSaaS\Comunidad\Tema;
use DigitalsiteSaaS\Comunidad\Template;
use DigitalsiteSaaS\Comunidad\Catecom;
use DigitalsiteSaaS\Comunidad\Areacom;
use DigitalsiteSaaS\Comunidad\Gradocom;
use DigitalsiteSaaS\Comunidad\Subtema;
use DigitalsiteSaaS\Comunidad\Rols;
use DigitalsiteSaaS\Comunidad\Page;
use DigitalsiteSaaS\Comunidad\Interes;
use DigitalsiteSaaS\Comunidad\Documento;
use DB;
use Input;
...
class ComunidadController extends Controller{
 ...
 public function create($id){
  return view('comunidad::comunidad.crear')->with('areas', $areas)->with('parametros', $parametros);
 }
 ...
 public function vernotas($id){
  ...
  return view('comunidad::comunidad.notas')->with('notas', $notas)->with('notascol', $notascol);
 }

 public function verdocumentos($id){
  $notas = DB::table('comunidad_documentos')->where('notas_id', '=', $id)->get();
  return view('comunidad::comunidad.documentos')->with('notas', $notas);
 }
 ...
 public function temas(){
  ...
  $temas = DB::table('temas_comunidades')
   ...
   ->get();
  return view('comunidad::comunidad.temas')->with('temas', $temas)->with('area', $area);
 }

 public function subtemas($id){
  $temas = DB::table('subtemas_comunidades')->where('subtemaid','=', $id)->get();
  return view('comunidad::comunidad.subtemas')->with('temas', $temas);
 }
 ...
 public function loginregister(Request $request){ 
  ...
  $responsed = $client->post('https://evadevback.lyl.com.co:8000/api/ws/eva_pin_register', [
  'form_params' => [
  'school' =>  'dsgfr',
  'user_id' =>  Input::get('identificador'),
  'pin' =>  Input::get('pin'),
  'token' =>  $token,],
 ]);
  ...
  return view('auth.loginpin')->with('plantilla', $plantilla)->with('menu', $menu)->with('cart', $cart)->with('total', $total)->with('subtotal', $subtotal)->with('error', $error)->with('ip', $ip)->with('ciudad', $ciudad)->with('pais', $pais);
 ...
 public function nota($id){
  ...
  $notas = DB::table('notas_comunidades')
   ...
   ->get();
  return view('comunidad::comunidad-notas')->with('notas', $notas)->with('plantilla', $plantilla)->with('menu', $menu)->with('media', $media)->with('subtotal', $subtotal)->with('total', $total)->with('cart', $cart)->with('documentos', $documentos);
 }
 ...
 public function notaweb($page){
  ...
  $notas = DB::table('notas_comunidades')
   ...
   ->get();
  return view('comunidad::comunidad-notasweb')->with('notas', $notas)->with('plantilla', $plantilla)->with('menu', $menu)->with('media', $media)->with('subtotal', $subtotal)->with('total', $total)->with('cart', $cart);
 }
...
public function creareva($id){
 ...
 return view('comunidad::comunidad.crear', compact('areas','parametros','temas','roles','data','grado','saber','clean','retorno','evas','recursos'));
...
$client = new Client(['http_errors' => false]);
 ...
 $temas = DB::table('temas_comunidades')->get();
 $retorno = DB::table('notas_comunidades')->where('id','=',2)->get();
 return view('comunidad::comunidad.editar', compact('areas','parametros','notas','temas','rols','notador','rolsas','roles','data','grado','saber','clean','retorno','evas','recursos'));
